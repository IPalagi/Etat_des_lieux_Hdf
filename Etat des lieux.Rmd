...
Cette partie a pour but d'identifier les année où des sécheresses hydrologiques ont eu lieu dans la région. Elle se base sur l'étude des débits de 6 stations hydrométriques utilisées comme référence dans les BSH (Bulletins de Situation Hydrologique).Ces stations sont complétées par d'autres stations de leurs bassins versants, celles étant étudiées chaque année dans les BSH. 

Pour le bassin Artois-Picardie, ces 6 stations de référence sont : 
...
```{r}
# Importation des données de débits pour les stations complémentaires 

# BV des fleuves côtiers 

q_E510 <- get_hydrometrie_obs_elab(     # La Slack à Rinxent 
  list(code_entite = "E5105710",
       date_debut_obs_elab = "1992-01-01",
       date_fin_obs_elab="2022-12-31",
       grandeur_hydro_elab = "QmJ")) %>% 
  select(code_station:resultat_obs_elab) %>% 
...
q_E520 <- get_hydrometrie_obs_elab(     # Le Wimereux à Wimille 
...
#### Pour les 6 stations de référence 
...
#### Pour les stations complémentaires 

```{r}
# BV des fleuves côtiers 

fdc_E510<-fdc(q_E510$resultat_obs_elab, lQ.thr=0.95,hQ.thr=NA, 
              main="La Slack à Rinxent ",xlab="Fréquence au dépassement", ylab="Débit (l/s)") #Q95 =56 l/s 

fdc_E520<-fdc(q_E520$resultat_obs_elab, lQ.thr=0.95,hQ.thr=NA, 
              main="Le Wimereux à Wimille",xlab="Fréquence au dépassement", ylab="Débit (l/s)") # #Q95 =64 l/s
```

Attention : les données de débits pour la Slack ne sont pas complètes, il n'ya pas de données entre 1993 et 1994 et entre 1995 et 1998, et il y a quelques trous (de quelques jours) entre 2000 et 2023. 

### Identification des années de sécheresse pour un seuil à Q90 

Les data frame sech_90_x contiennent le nombre de jours de sécheresse par an pour une station donnée.  
...
sech_90_E530<-q_E530 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E530=sum(resultat_obs_elab<381)) 
...
sech_90_D013<-q_D013 %>% 
 group_by(annee) %>% 
  summarise(duree_sech_D013=sum(resultat_obs_elab<250)) 
...
sech_95_E530<-q_E530 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E530=sum(resultat_obs_elab<328)) 

# La Slack 

sech_95_E510<-q_E510 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E510=sum(resultat_obs_elab<56))

# Le Wimereux 

sech_95_E520<-q_E520 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E520=sum(resultat_obs_elab<64))
...
sech_95_D013<-q_D013 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_D013=sum(resultat_obs_elab<213))
```
...
sech_95_E351<-q_E351 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E351=sum(resultat_obs_elab<846))
...
sech_95_E540<-q_E540 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E540=sum(resultat_obs_elab<7844))
...
sech_95_E647<-q_E647 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_E647=sum(resultat_obs_elab<19644))
```
...
sech_95_H740<-q_H740 %>% 
  group_by(annee) %>% 
  summarise(duree_sech_H740=sum(resultat_obs_elab<8120))
```


Agrégation dans un même data frame de toutes les durées de sécheresse : 

```{r}
duree_sech<-data.frame(annee=sech_95_D013$annee, D013=sech_95_D013$duree_sech_D013,     
                       E351=sech_95_E351$duree_sech_E351, 
                       E520=sech_95_E520$duree_sech_E520,
                       E530=sech_95_E530$duree_sech_E530,
                       E540=sech_95_E540$duree_sech_E540,
                       E647=sech_95_E647$duree_sech_E647,
                       H740=sech_95_H740$duree_sech_H740)
```
